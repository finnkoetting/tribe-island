...
const bushId = (rng: () => number) => `berry_${Math.floor(rng() * 1e9).toString(16)}`;
...
const makeBush = (id: string, pos: Vec2): Building => ({
    id,
    type: "berry_bush",
    pos,
    level: 1,
...
const ALLOWED_TILES = new Set<WorldTileId>(["meadow", "forest", "grass"]);

export function spawnBerryBushes(st: GameState, count: number, rng: () => number): GameState {
    ...
    for (const idx of shuffled) {
        ...
        if (!canPlaceAt(next, pos, "berry_bush")) continue;

        const bush = makeBush(bushId(rng), pos);
        next = placeAt(next, bush);
        spawned += 1;
    }
